...
function NavBar() {
  ...
  return (
    <>
      {isMenu && (
        <div className="absolute z-10 w-full h-[95vh] bg-black bg-opacity-85 grid place-content-center">
          <ul className="text-xl text-center font-bold flex flex-col gap-4 items-center">
            ...
            <li className="flex gap-4">
              <Button
                ...
                onClick={closeMenuClickHandler}
              >
                <Link to="/auth?state=login" className="text-lg">
                  Login
                </Link>
              </Button>
            </li>
            <li className="flex gap-4">
              <Button
                ...
                onClick={closeMenuClickHandler}
              >
                <Link to="/auth?state=create-account" className="text-lg">
                  Create account
                </Link>
              </Button>
            </li>
      ...
      <div className="container mx-auto flex items-center justify-between p-1 relative">
        <Link to="/" className="flex items-center">
          <img
            className="w-12 h-12"
            src="https://i.ibb.co/S0dWnZC/file.png"
            alt="Logo image"
          />
        ...
        <div className="hidden md:flex flex-grow justify-around">
          <input
            ...
            placeholder="Search"
            name="Search"
            className="border w-1/2 border-black rounded-md outline-none py-2 px-3 justify-center"
            id=""
          />
        </div>
        ...
        {login ? (
          ...
          <>
            <nav className="flex gap-4 px-4">
              <Link
                to="/auth?state=create-account"
                className="hidden md:flex bg-blue-500 text-white px-5 py-2 rounded-lg hover:bg-blue-600 transition duration-300"
              >
                Create account
              </Link>
              ...
              <Link
                to="/auth?state=login"
                className="hidden md:flex bg-green-500 text-white px-5 py-2 rounded-lg hover:bg-green-600 transition duration-300"
              >
                Log in
              </Link>
              <button onClick={() => setIsMenu(!isMenu)} className="md:hidden">
                {isMenu ? <IoMdClose size={25} /> : <IoMenu size={25} />}
              </button>
            </nav>{" "}
          </>
        )}
...
const profie_picture = (
  <Avatar
    src="https://dub.sh/TdSBP0D"
    alt="profile-picture"
    className="w-8 h-8"
  />
); // declaring profile_picture for later use
...
function UserLoginIcon() {
  ...
  return (
    <Popover>
      <Popover.Trigger className="text-sm">
        <List.Item>
          <List.ItemStart>{profie_picture}</List.ItemStart>
